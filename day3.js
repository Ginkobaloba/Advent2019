...
function findArraySize(path1, path2) {
    ...
    for (i = 0; i < path1.length; i++) {
        if (path1[i].charAt(0) === 'R') {
            x1 = x1 + parseInt(path1[i].substr(1));
            if (x1 > xMax1) {
                xMax1 = x1;
            };
        }
        else if (path1[i].charAt(0) === 'L') {
            x1 = x1 - parseInt(path1[i].substr(1));
            if (x1 < xMin1) {
                xMin1 = x1;
            };
        }
        else if (path1[i].charAt(0) === 'U') {
            y1 = y1 + parseInt(path1[i].substr(1));
            if (y1 > yMax1) {
                yMax1 = y1;
            };
        }
        else if (path1[i].charAt(0) === 'D') {
            y1 = y1 - parseInt(path1[i].substr(1));
            if (y1 < yMin1) {
                yMin1 = y1;
            };
    ...
    for (i = 0; i < path1.length; i++) {
        if (path2[i].charAt(0) === 'R') {
            x2 = x2 + parseInt(path2[i].substr(1));
            if (x2 > xMax2) {
                xMax2 = x2;
            };
        }
        else if (path1[i].charAt(0) === 'L') {
            x2 = x2 - parseInt(path1[i].substr(1));
            if (x2 < xMin2) {
                xMin2 = x2;
            };
        }
        else if (path1[i].charAt(0) === 'U') {
            y2 = y2 + parseInt(path1[i].substr(1));
            if (y2 > yMax2) {
                yMax2 = y2;
            };
        }
    ...
    console.log(xMin1, xMax1)
    console.log(xMin2, xMax2)
    console.log(yMin1, yMax1)
    console.log(yMin2, yMax2)
    console.log(absMinX, absMaxX)
    console.log(absMinY, absMaxY)

    var width = Math.abs(absMaxX) + Math.abs(absMinX)
    var height = Math.abs(absMaxY) + Math.abs(absMinY)

    var boxInfo = { width, height, absMinX, absMinY }
